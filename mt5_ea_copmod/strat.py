...
def strategy_one(symbol, timeframe, pip_size):
    ...
    data_df = get_and_transform_mt5_data(symbol=symbol, 
                                         timeframe=timeframe, 
                                         number_of_candles=2, 
                                         pip_size=pip_size)
    print("\n*----------------------------------"+\
          "--------------------------------------*")
    lst_dropcols = ['open','tick_volume','real_volume', 
                    'high','low', 'pip_distance']
    print(data_df.drop(lst_dropcols, axis=1))
    print("_-------------------------------------"+\
          "-----------------------------------_\n")
    ...
    create_new_order(decision_outcome=decision, 
                     candle_dataframe=data_df, 
                     pip_size=pip_size, 
                     symbol=symbol)
    return "Completed"
...
#    df_data['RedOrGreen'] = numpy.\
#        where((df_data['open'] < df_data['close']), 'Green', 'Red')
...
def get_and_transform_mt5_data(symbol, timeframe, number_of_candles, pip_size):
    ...
    df_data['RedOrGreen'] = numpy.\
        where((df_data['open'] < df_data['close']), 'Green', 'Red')
    # Calculate trade_high (high price + pip size)
    df_data['trade_high'] = df_data['high'] + pip_size
    # Calculate trade_low (low price - pip size)
    df_data['trade_low'] = df_data['low'] - pip_size
    # Calculate the number of pips between trade_high and trade_low
    df_data['pip_distance'] = \
        (df_data['trade_high'] - df_data['trade_low'])/pip_size
    # Return the data frame to the user
    return df_data
...
def make_decision(candle_dataframe):
    ...
    if(candle_dataframe.iloc[-2]['RedOrGreen'] != \
       candle_dataframe.iloc[-1]['RedOrGreen']):
        return "DoNothing"
    # Test if both are Green
    elif(candle_dataframe.iloc[-1]['RedOrGreen'] == "Green"):
        return "Green"
    # Test if both are Red
    elif (candle_dataframe.iloc[-1]['RedOrGreen'] == "Red"):
        return "Red"
...
def create_new_order(decision_outcome, candle_dataframe, pip_size, symbol):
    ...
    if decision_outcome == "DoNothing":
        ...
        stop_loss = first_row['trade_low'] - pip_size*first_row['spread']
        # Calculate the order buy_stop (trade_high of previous candle)
        buy_stop = first_row['trade_high'] + pip_size*first_row['spread']
        # Calculate the order take_profit (2 times the pip distance)
        ...
        mt5_interface.place_order("BUY_STOP", symbol, 0.01, 
                                  buy_stop, stop_loss, take_profit, comment)
        return
    elif decision_outcome == "Red":
        # Calculate the order stop_loss (trade_high of previous candle)
        stop_loss = first_row['trade_high'] + pip_size*first_row['spread']
        # Calculate the order buy_stop (trade_low of previous candle)
        buy_stop = first_row['trade_low'] - pip_size*first_row['spread']
        # Calculate the order take_profit (2 times the pip distance)
        num_pips = first_row["pip_distance"] * 2 * pip_size  # Convert pip_distance back into pips
        take_profit = buy_stop - num_pips
        # Add in an order comment
        comment = "Red Order"
        # Send order to place_order function in mt5_interface.py
        mt5_interface.place_order("SELL_STOP", symbol, 0.01, buy_stop, 
            ...
            if new_stop_loss < order[11]:
                ...
                mt5_interface.\
                    modify_position(order_number=order_number, 
                                    ...
                                    new_stop_loss=new_stop_loss,
                                    new_take_profit=new_take_profit)
    elif (order[13] - order[10]) >= trailing_stop_pips:
        ...
        if isUpdate:
            ...
            mt5_interface.\
                modify_position(
                    ...
                    symbol=symbol, 
                    new_stop_loss=new_stop_loss,
                    new_take_profit=order[10] + 3*trailing_stop_pips)
    else:
        if order[5] == 0: # long position
            new_stop_loss = order[10] - trailing_stop_pips
            isUpdate = new_stop_loss > order[11]
        else: # short position
            new_stop_loss = order[10] + trailing_stop_pips
            isUpdate = new_stop_loss < order[11]
