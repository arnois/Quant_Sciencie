...
def dt_delims_train_test(str_end_train, n_yr_train, n_month_test):
    # Train start date
    dt_start_train = pd.Timestamp(
        str_end_train - np.timedelta64(52*n_yr_train,'W')).replace(day=1)
    str_start_train = dt_start_train.strftime('%Y-%m-%d')
    # Test start date
    str_start_test = pd.Timestamp(str_end_train + np.timedelta64(28,'D')).\
        replace(day=1)
    # Test end date
    str_end_test = str_start_test + pd.DateOffset(months=1*n_month_test)
    str_start_test = str_start_test.strftime('%Y-%m-%d')
    str_end_test = (str_end_test.replace(day=1)).strftime('%Y-%m-%d')
    str_end_train = pd.Timestamp(str_end_train).strftime('%Y-%m-%d')
...
def get_train_test_split_dates(df, n_yr_train, n_month_test, n_month_test_roll):
    ...
    dt_first = (df.index[0]+np.timedelta64(12*n_yr_train,'M')).replace(day=1)
    dt_1stEoM = pd.Timestamp(last_day_of_month(dt_first))
    n_total_months = int(
        (df.index[-1]-dt_1stEoM)/np.timedelta64(1,'M')
        ) - n_month_test + 1
    colnames = ['train_st','train_ed','test_st','test_e']
    df_dtsplit = pd.DataFrame(columns=colnames)
    dt_train_end = dt_1stEoM #dt_first
    for i in range(n_total_months):
        dt_train_end = dt_1stEoM+np.timedelta64(1*i*n_month_test_roll,'M')
        #str_end_train = np.datetime_as_string(last_day_of_month(dt_train_end)+\
        #                                  np.timedelta64(1,'D'))
