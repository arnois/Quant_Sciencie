...
# User input example
dates = ['06 Jul 1933', '05 Jun 1933', '28 Feb 1921']

# Split dates by day,month,year
split_dates = [s.split(' ') for s in dates]
df_sdates = pd.DataFrame(split_dates, columns = ['d','m','y'])
df_sdates['input'] = dates

# dictionary to map month to number
dic_m2n = dict(zip(['Jan','Feb','Mar','Apr','May','Jun',
                    'Jul','Aug','Sep','Oct','Nov','Dec'],
                   range(1,13)))

# Parse strings to ints
df_sdates['m'] = df_sdates['m'].map(dic_m2n)
df_sdates[['d','y']] = df_sdates[['d','y']].astype(int)

# MultiIndex
tmpidx = pd.MultiIndex.from_frame(df_sdates[['y','m','d']])

# Dates as dataframe with multiindex
df_dates = pd.DataFrame(dates, columns=['input'], index=tmpidx)

# Sorted dates in chronologically order
df_dates.sort_index()
df_dates.sort_values(by=[('y','m','d')])

#%% Palindrome Permutation
str_input = 'taco cat is' # 'anita lava la tina'
u_chars = set(str_input.replace(' ',''))
n_chars = [(c,str_input.count(c)) for c in u_chars]
odds = []
evens = []
res = False
for elem in n_chars:
...
#%% Longest Substring wo Replacement
class tmpFunc:
    def getPossibleNextChars(self, dic_chars, left_char):
    
        return [c for c in dic_chars.keys() 
                if any([n<m for n in dic_chars[left_char[-1]] 
                        for m in dic_chars[c]]) and c not in left_char]
    
def getSubstrings(s: str) -> list:
    # Appearance order of each char
    enum_s = [(c,i) for i,c in enumerate(s)]

    # Unique char with order of appearance
    u_s = dict(zip(set(s), [[] for x in range(len(set(s)))]))
    for elem in enum_s:
        u_s[elem[0]].append(elem[1])
        
    # If only one unique char, then it is it
    if len(u_s)<=1:
        return list(u_s.keys())

    # Loop through all unique ordered chars
    list_subtrings = []
    for k in u_s.keys():
        # Get the 2-char substrings
        substr = k
        tmpac = tmpFunc().getPossibleNextChars(u_s,k)
        tmpac = [c for c in tmpac if substr+c in s]
        lsubtr = [substr+q for q in tmpac if substr+q in s]
        # Get all further n-char substrings
        while lsubtr != []:
            # Concatenate following chars to get substrings
            tmpdic = {}
            for i in lsubtr: 
                tmpdic.setdefault(i, tmpFunc().getPossibleNextChars(u_s,i))
            for r,v in tmpdic.items(): 
                tmpdic[r] = [q for q in v if r+q in s]
            keys2remove = [c for c,w in tmpdic.items() if w == []]
            # Here: tmpdic keys tells the substrings 
            [list_subtrings.append(q) for q in tmpdic.keys()]
            [tmpdic.pop(c) for c in keys2remove]
            lsubtr = [r+t[0] for r in tmpdic.keys() 
                                for t in tmpdic.values() if r+t[0] in s]
    
    return list_subtrings
    
def lengthOfLongestSubstring(s: str) -> int:
    # Check for string having already unique chars
    setstr = set(s)
    if len(setstr) == len(s):
        return len(s)
    
    # All possible subtrings
    lstSubstrings = getSubstrings(s)
    
    # Length of all substrings
    substr_len = [len(c) for c in lstSubstrings]
    
    return max(substr_len)

tests = ['ehespkahcyng', 'qpxrjxkitzyxacbhhkicqc', 
         'gehmbfqmozbpripibusbezagafqtypz', 'abcabcbb', 'bbbbb', 'abba',
         'pwwkew']
for strinput in tests:
    print(f'{strinput}: {lengthOfLongestSubstring(strinput)}')

# Function to get all possible next chars
def getPossibleNextChars(dic_chars, left_char):
...
# User input case
s = 'ehespkahcyng'
s = 'qpxrjxkitzyxacbhhkicqc'
s = 'gehmbfqmozbpripibusbezagafqtypz'
# Appearance order of each char
enum_s = [(c,i) for i,c in enumerate(s)]
...
for k in u_s.keys():
    substr = k
    tmpac = getPossibleNextChars(u_s,k)
    tmpac = [c for c in tmpac if substr+c in s]
    lsubtr = [substr+q for q in tmpac if substr+q in s]
    while lsubtr != []:
        tmpdic = {}
        for i in lsubtr: tmpdic.setdefault(i, getPossibleNextChars(u_s,i))
        for r,v in tmpdic.items(): tmpdic[r] = [q for q in v if r+q in s]
        keys2remove = [c for c,w in tmpdic.items() if w == []]
        [list_subtrings.append(q) for q in tmpdic.keys()]
        [tmpdic.pop(c) for c in keys2remove]
        max_len = max(max_len, max([len(c) for c in lsubtr]))
        lsubtr = [r+t[0] for r in tmpdic.keys() for t in tmpdic.values() if r+t[0] in s]

max(max_len,max([len(x) for x in list_subtrings]))
           

# Iter over every unique char
set_substr = set()
for k,v in u_s.items():
    lsubstr = ''
    # Get all possible chars afterwards
    ac = [c for c in u_s.keys() if any([n<m for n in v for m in u_s[c]]) and c!=k]
    # all possible 2-char substrings
    a2c = [k+c for c in ac if k+c in s]
    # Possible chars after second char
    for tmp in a2c:
        acc = [c for c in u_s.keys() if any([n<m for n in u_s[tmp[-1]] for m in u_s[c]]) and c not in tmp]
        [p+q for p in a2c for q in acc]
        
    substr = ''
    substr += k
    for c in u_s.keys():
        isCharAfter = any([n<m for n in u_s[substr[-1]] for m in u_s[c]])
        isCharDup = c in substr
        if isCharAfter and not isCharDup:
            substr += c
        else:
            continue
            
        if substr in s:
            set_substr.add(substr)
        else:
            if substr[:-2] != '':
                tmp_str = substr[:-2]+c
                if tmp_str in s:
                    set_substr.add(tmp_str)
                substr = substr[:-2]
            else:
                substr = substr[:-1]
                 
max([len(s) for s in set_substr])
            

# Occurrence of every char
lst_char_n = [(i,c, s.count(c)) for i,c in enumerate(u_s)]
tmpset = {}
for elem in set(s):
    tmpset[s.index(elem)] = elem
    
# Loop through all unique ordered chars
substr=''
iterlst = list(tmpset.keys())
iterlst.sort()
# Continuous-constrained Power set 
powSet = []
for j in range(0,len(iterlst)-1):
    for i in range(j+1,len(iterlst)+1):
        if len(iterlst[j:i])>1:
            powSet.append(iterlst[j:i])    
max_len = 0
substr = ''
lst_substr = []
for v in powSet:
    substr = ''.join([tmpset[x] for x in v])
    if substr in s:
        lst_substr.append(substr)
...
len_s = len(s)
for c in s:
    next_chr_idx = s.index(c)+1
    if next_chr_idx >= len_s:
        next_char = ''
    else:
        next_char = s[next_chr_idx]
    if s.count(c)>1:
        if c == last_chr or c == next_char: # consecutive repeated char
            consecutive_chars.add(c)
        else:
            rep_chars.add(c)
    last_chr = c
    
# partition by consecutives
lstsubstr = []
for e in consecutive_chars:
    tmpart = s.partition(e)
    lstsubstr.append(''.join(tmpart[:2]))
    if tmpart[2].count(tmpart[1]) > 1:
        if tmpart[2].count(tmpart[1]) == len(tmpart[2]):
            lstsubstr.append(tmpart[2][0])
        else:
            lstsubstr.append(tmpart[2].partition(tmpart[1])[2])
    else:
        lstsubstr.append(tmpart[2])
        
# partition by other repeated chars
for e in rep_chars:
    tmpart = s.partition(e)
    lstsubstr.append(''.join(tmpart[:2]))
    if tmpart[2].count(tmpart[1]) > 1:
...
# check for substrings in substrings
for elem in lstsubstr:
    copylst = lstsubstr.copy()
    copylst.remove(elem)
    isElemRep = [s in elem for s in copylst if len(s)>1]
    if any(isElemRep):
        lstsubstr.remove(elem)
        
max([len(substr) for substr in lstsubstr])
